...
class RandCropByPosNegLabeldWithResAdjust(Randomizable):
    def __init__(self, image_key, label_key, spatial_size, pos, neg, num_samples, image_threshold):
    ...
    def randomize(self, data):
        ...
        if torch.rand(1).item() < self.pos / (self.pos + self.neg):
            ...
            fg_coords = torch.nonzero(img_data[0, 0] > self.image_threshold)
            ...
            bg_coords = torch.nonzero(img_data[0, 0] <= self.image_threshold)
        ...
        start_coord = torch.clamp(center_coord - spatial_tensor // 2, torch.tensor([0, 0, 0]), torch.tensor([depth, height, width]))
        ...
        start_coord = tuple(start_coord.tolist())
        end_coord = tuple(end_coord.tolist())
    ...
    def __call__(self, data):
        ...
        for _ in range(self.num_samples):
            ...
            cropped_image = sample[self.image_key][..., self.start_coord[0]:self.end_coord[0], self.start_coord[1]:self.end_coord[1], self.start_coord[2]:self.end_coord[2]]
            ...
            cropped_label = sample[self.label_key][..., label_start_coord[0]:label_end_coord[0], label_start_coord[1]:label_end_coord[1], label_start_coord[2]:label_end_coord[2]]
