...
# import monai.networks.nets.swin_unetr as SWU
...
class MyUpscaleSwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            upsample_end: whether to only upsample the final output or upsample the input and use a skip connection to attach it to the final layers input
        ...
        if self.upsample_end:
            ...
            self.out = UnetOutBlock(spatial_dims=spatial_dims, in_channels=16, out_channels=out_channels)
        else:
            self.decoder0_upsampler = UnetrUpBlock(
                spatial_dims=spatial_dims,
                in_channels=feature_size,
            ...
            #     Conv3DBlock(feature_size, feature_size),
            #     SingleDeconv3DBlock(feature_size, feature_size),
            # )
        ...
        #     Conv3DBlock(16, 16),
        #     SingleConv3DBlock(16, out_channels, 1),
        # )
    ...
    def forward(self, x_in):
        ...
        dec5 = self.encoder10(hidden_states_out[4])
        dec4 = self.decoder5(dec5, hidden_states_out[3])
        dec3 = self.decoder4(dec4, enc4)
        dec2 = self.decoder3(dec3, enc3)
        dec1 = self.decoder2(dec2, enc2)
        dec0 = self.decoder1(dec1, enc1)

        if not self.upsample_end:
            x_up = self.upsampler(x_in)
            out = self.decoder0_upsampler(dec0, x_up)
            logits = self.out(out)
        else:
            out = self.upsampler(dec0)
            logits = self.out(out)
